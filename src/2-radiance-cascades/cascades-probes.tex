...
Then, we present the main result of \acrshort{rcs}, which enables significantly more efficient encoding of the radiation field by taking advantage of the key observations of the penumbra criterion. 
Finally, we describe the cascade merging algorithm, which efficiently computes for the radiation field. 
...
We simply prescribe $\alpha=2$ for simplicity in presentation, but this is not required in general for \acrshort{rcs}.
...
The inefficiency and failure of these discretizations is because the angular step (arc length) between two cast rays is not constant, and grows smaller further from the point. 
This causes either inefficient computation near the point or incorrect solutions far from the point. 
Conversely, the radiation field discretization in (iii), the \acrshort{rcs} discretization, efficiently resolves near field while accurately resolving far-field.
By using radiance intervals with increasing angular discretization, the angular step is roughly constant between any given radiance interval, and so the solution is both efficiently computed and accurate.
...
Although this discretization scheme is more efficient or more accurate than the full ray-casting discretizations, it only utilizes one part of the penumbra criterion.
...
\begin{figure}[!htbp]
    ...
    (i) shows the naive approach to \acrshort{rcs}, applying a full set of radiance intervals to each point.
    (ii) demonstrates the compression of nearby same-range radiance intervals, the method employed in true \acrshort{rcs}.
...
Importantly, $A$ is a straight forward calculation that requires the radiation contribution at the end point, which is solved for recursively (as discussed in~\cref{subsubsection:ri-merging}).
...
\begin{enumerate}
    \item Identify the neighboring probes in the next lower cascade. 
    In~\cref{fig:interval-compression}(ii), this consists of all shown probes in $C_1$.
...
\begin{figure}[!htbp]
    ...
    Then, these sums are bi-linearly interpolated from their respective points onto the endpoint of the target ray.
    }
...
The second step complicates this slightly, but is still extremely Parallelizable. 
...
For the 2-D case with the branching ratio $\alpha=2$, the majority of probes require communication from 4 probes from the adjacent higher cascade (3-D requires 8).
An example graph for the 2-D case with $\alpha=2$ is shown in Appendix~\ref{appendix:merge-graph}.
