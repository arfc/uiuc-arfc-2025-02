...
Benchmark cases for \acrshort{rc} must be chosen to specifically show the advantages of the method and directly test for known problems in other methods, such as ray-effects in $S_N$.
To be considered successful, we will need to implement \acrshort{rc} in such a way that not only converges to the correct solution, but can also do so faster than proven methods such as \gls{sn}.
Or, alternatively, must be proven to scale well using GPU parallelism.
We propose the following benchmark cases be used for the development and testing of \acrshort{rc}.

The application of \acrshort{rc} to the \gls{bte} has been largely proven by Osborne and Sannikov~\cite{osborneRadianceCascadesNovel2025}, though with a particular application to photon transport in astrophysics.
...
\begin{enumerate}
    ...
    \item The source term is dependent on the flux itself in both multiplying (fission) and nonmultiplying systems.\label{second difference}
\end{enumerate}
\Cref{subsubsec:kobayashi} gives a general descriptions for the proposed benchmarks along with reasons behind their selection.

\subsubsection{Streaming and Fixed Source: Kobayashi Benchmark}\label{subsubsec:kobayashi}
...
\end{table}
The ultimate goal will be to report on \acrshort{rc}'s capabilities on the 3D problems as they were originally published.
