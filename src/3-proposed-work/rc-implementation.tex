...
Rather than immediately implementing \acrshort{rcs} in the general case, we propose to begin with a simple implementation applied to no more than two ``toy'' problems.
...
\begin{enumerate}
    \item Provide an opportunity to practice implementing \acrshort{rcs} on a small, well-defined problem before moving to a generalized code base.
...
We investigated four codes to potentially implement \acrshort{rcs} in: OpenMC, OpenSN, DexRT, and OpenMOC.
\paragraph{OpenMC}
OpenMC~\cite{romanoOpenMCStateoftheartMonte2015d} has a mature code base featuring CSG and MGXS modules, and so it satisfies both required prerequisites.
...
Unfortunately, we believe OpenMC is not the correct code to implement \acrshort{rcs} in, as it is primarily a Monte Carlo code. 
While OpenMC does have a deterministic solver, \acrshort{trrm} solver, this is only supported due to the inherently stochastic nature of \acrshort{trrm}. 
\acrlong{rcs} do not have a stochastic element, and thus do not belong in OpenMC.
...
OpenSN explicitly supports unstructured meshes, and should be capable of supporting structured meshes through input files, but this feature is unclear. 
OpenSN is actively maintained, and recently underwent a massive update to the code base, in which a python interface was added. 
Although OpenSN is now run through it's python interface, the backend is entirely C++, and the interfacing is not done through Cython or SWIG.
While OpenSN is an actively maintained deterministic research code, we believe this code is not the correct code to implement in.
The goal of OpenSN is to investigate different sweeping algorithms for the discrete ordinates method, and so implementing \acrshort{rcs} does not align with the goal of the code. 

\paragraph{DexRT}
DexRT~\cite{osborneRadianceCascadesNovel2025} is a non-LTE radiative transfer code, and is the first published \acrshort{rcs} implementation for physics applications. 
...
Additionally, as DexRT is a implementation of \acrshort{rcs} for a physics application, it is adaptable to solving the \acrshort{lbe}.
Despite mostly satisfying the prerequisites, there are a few notable considerations for implementing in DexRT:
\begin{enumerate}
    \item DexRT is in rapid but sporadic development, and is maintained by a single person.
    \item DexRT solves for non-LTE radiative transfer, and this problem is heavily baked into the code base. An abstraction to also solve the \acrshort{lbe} could be tricky to implement, and may not be supported by Osborne, as the problems are very different.
\end{enumerate}
...
\paragraph{OpenMOC}
OpenMOC~\cite{boydOpenMOCMethodCharacteristics2014a} is an open source \acrshort{moc} code developed by Dr. Forget's group at the Massachusetts Institute of Technology.
...
For example, the code uses SWIG for python bindings of C++ code, which adds a complicated step to the build system.
...
Additionally, OpenMOC is outdated and will require code maintenance and updates prior to implementing \acrshort{rcs}. This will be discussed further in~\cref{subsubsec:implementation-outline}.
That said, we believe that of the four codes investigated, OpenMOC is clearly the code to implement \acrshort{rcs} into for a few reasons. 
...
Additionally, the intent of OpenMOC was to investigate and demonstrate long characteristic methods for solving the \acrshort{lbe}.
OpenMOC is also an open-source package, that is nearly in the code graveyard of github, and could be revived if we implement \acrshort{rcs} into OpenMOC.
...
This section assumes we will be implementing into OpenMOC, and will layout a rough estimate on the work for the initial \acrshort{rcs} implementation. 
...
Before implementing \acrshort{rcs}, we believe OpenMOC should be able to do all the things it's documentation says it can do, and so these must be updated.

\paragraph{Desired Updates}
There are \emph{several} desired updates to OpenMOC that we believe will be extremely helpful for implementing \acrshort{rcs}:
\begin{enumerate}
    \item Code organization: The C++ source code is currently all stored in the same directory. We propose organizing the code into logical sub-directories, similar to the organization of INL's MOOSE framework.
    \item Code complexity: The C++ source integration into Python using SWIG complicates the code and build system. We propose separating the C++ source and Python interface, similair to how OpenMC works.
    ...
    \item Linear Algebra: OpenMOC has a \acrshort{cmfd} solver implemented, where the data storage (sparse matrix) and the solver itself are written natively. 
    For the sake of conciseness and performance, we propose bringing in PETSc for the sparse linear algebra that \acrshort{cmfd} requires. 
    This would drastically decrease the volume of code written for the \acrshort{cmfd} solver, and entirely remove the need of the \texttt{vector} class. 
    Additionally, using PETSc guarantees adequate performance, scalability, and portability (PETSc supports Kokkos).
...
For the initial implementation, we propose the following required features:
\begin{enumerate}
    ...
    \item Steady-state 2D \acrshort{rcs} solver
    \item Capability to handle both multiplying and fixed-source problems
    \item The standard \acrshort{rcs} merging algorithm and Bilinear fix merging algorithm
\end{enumerate}
