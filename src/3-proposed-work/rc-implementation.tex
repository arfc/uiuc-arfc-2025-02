...
Rather than immediately implementing \glspl{rcs} in the general case, we propose to begin with a simple implementation applied to no more than two ``toy'' problems.
...
\begin{enumerate}
    \item Provide an opportunity to practice implementing \glspl{rcs} on a small, well-defined problem before moving to a generalized code base.
...
We investigated four codes to potentially implement \glspl{rcs} in: OpenMC, OpenSN, DexRT, and OpenMOC.
\paragraph{OpenMC}
...
Unfortunately, we believe OpenMC is not the correct code to implement \glspl{rcs} in, as it is primarily a Monte Carlo code. 
...
The goal of OpenSN is to investigate different sweeping algorithms for the discrete ordinates method, and so implementing \glspl{rcs} does not align with the goal of the code. 

\paragraph{DexRT}
DexRT~\cite{osborneRadianceCascadesNovel2025} is a non-LTE radiative transfer code, and is the first published \glspl{rcs} implementation for physics applications. 
...
Additionally, as DexRT is a implementation of \glspl{rcs} for a physics application, it is adaptable to solving the \gls{lbe}.
...
Additionally, OpenMOC is outdated and will require code maintenance and updates prior to implementing \glspl{rcs}. This will be discussed further in~\cref{subsubsec:implementation-outline}.
That said, we believe that of the four codes investigated, OpenMOC is clearly the code to implement \glspl{rcs} into for a few reasons. 
...
OpenMOC is also an open-source package, that is nearly in the code graveyard of github, and could be revived if we implement \glspl{rcs} into OpenMOC.
...
This section assumes we will be implementing into OpenMOC, and will layout a rough estimate on the work for the initial \glspl{rcs} implementation. 
...
Before implementing \glspl{rcs}, we believe OpenMOC should be able to do all the things it's documentation says it can do, and so these must be updated.

\paragraph{Desired Updates}
There are \emph{several} desired updates to OpenMOC that we believe will be extremely helpful for implementing \glspl{rcs}:
\begin{enumerate}
    ...
    For the sake of conciseness and performance, we propose bringing in PETSc for the sparse linear algebra that \gls{cmfd} requires. 
...
\begin{enumerate}
    ...
    \item Steady-state 2D \glspl{rcs} solver
    \item Capability to handle both multiplying and fixed-source problems
    \item The standard \glspl{rcs} merging algorithm and Bilinear fix merging algorithm
\end{enumerate}
